...
#for qps in 100000 200000 400000 600000 800000 1000000 1200000 1400000 1600000; do
	for itrd in 1 2 20 50 100 150 200 250 300 350 400; do 
		...
		ssh $mcd_server "mkdir -p ~/ixgbe_logs/; sudo ethtool -C $iface rx-usecs $itrd; sudo wrmsr -a 0x199 0x1900; ./flush_ixgbe_logs.sh &> /dev/null; sudo ./read_rapl_start_c8220.sh";
