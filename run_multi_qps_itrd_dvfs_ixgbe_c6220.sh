	...
	for itrd in 1 2 20 50 100 150 200 250 300 350 400; do 
		for dvfs in 0xc00 0x1100 0x1500 0x1900 0x1c00; do 
			...
			# start memcached server
	                ssh $mcd_server "taskset -c 0-6,8-14 ~/memcached/memcached -u nobody -t 16 -m 32G -c 8192 -b 8192 -l $mcd_server -B binary > mcd_dump 2>&1 < /dev/null &"
			sleep 1;
			...
			ssh $mcd_server "mkdir -p ~/ixgbe_logs/; sudo ethtool -C $iface rx-usecs $itrd; sudo wrmsr -a 0x199 $dvfs; ./flush_ixgbe_logs_c6220.sh &> /dev/null; sudo ./read_rapl_start_c6220.sh";
			
			# start mutilate run
			taskset -c 0 ~/mutilate/mutilate --binary -s $mcd_server --noload --agent={10.10.1.3,10.10.1.4,10.10.1.5} --threads=1 --keysize=fb_key --valuesize=fb_value --iadist=fb_ia --update=0.25 --depth=4 --measure_connections=4 --measure_qps=2000 --qps=$qps --time=30; 
			
			# get rapl readings for this run
	                ssh $mcd_server "sudo ./read_rapl_c6220.sh; ./cat_ixgbe_logs_c6220.sh ixgbe_logs/; sudo killall memcached";
	                for i in 3 4 5; do
