...
for i in "${!node_list[@]}"; do
	...
	ssh $myself@${node_list[i]} 'if [ -f ".ssh/id_rsa.pub" ]; then echo "id_rsa.pub exists.. skipping ssh-keygen"; else ssh-keygen -t rsa -f ~/.ssh/id_rsa -N ""; fi';
...
#	ip_2=$(ssh $myself@${node_list[i]} 'ifconfig | grep -B1 130.127.133.67 | grep inet | grep -oP "inet \K(\d+\.\d+\.\d+\.\d+)"')
	#echo "$id_rsa";
...
#			ssh $myself@${node_list[j]} "ssh-keyscan -H ${node_list[i]} >> .ssh/known_hosts; ssh-keyscan -H $ip_1 >> .ssh/known_hosts; ssh-keyscan -H $ip_2 >> .ssh/known_hosts";
			ssh $myself@${node_list[j]} "ssh-keyscan -H ${node_list[i]} >> .ssh/known_hosts; ssh-keyscan -H $ip_1 >> .ssh/known_hosts";
		fi
	done
	echo "";
done
...
#		ip_2=$(ssh $myself@${node_list[j]} 'ifconfig | grep -B1 130.127.133.67 | grep inet | grep -oP "inet \K(\d+\.\d+\.\d+\.\d+)"')
