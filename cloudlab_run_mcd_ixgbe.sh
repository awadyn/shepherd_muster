...
ssh $myself@$mcd_server 'for i in {0..15}; do cat /proc/ixgbe_stats/core/$i; echo; done';
...
ssh $myself@$mcd_server 'for i in {0..15}; do cat /proc/ixgbe_stats/core/$i > ~/intlog_logs/$i.intlog; done';

scp -r $myself@$mcd_server:~/intlog_logs/* mcd_ixgbe_logs/$qps/;
scp -r $myself@$mutilate_server:~/mutilate_latency mcd_ixgbe_logs/$qps/mutilate_latency_"$dvfs";
for i in {0..15}; do
mv mcd_ixgbe_logs/$qps/"$i".intlog mcd_ixgbe_logs/$qps/"$dvfs"_"$i".intlog;
done;
