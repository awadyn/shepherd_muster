...
func (intlog_m *intlog_muster) init() {
	...
	for sheep_id, sheep := range(intlog_m.pasture) {
		...
		log_c := log{id: log_id}
		log_c.init(intlog_m.buff_max_size, intlog_m.ixgbe_metrics, 3)
		...
		intlog_m.pasture[sheep_id].controls[ctrl_itr_shared.id] = &ctrl_itr_shared
	}
}
