...
type ctrl_muster struct {
	intlog_muster
}

type ctrl_shepherd struct {
	intlog_shepherd
	ctrl_musters map[string]*ctrl_muster 
}


func (ctrl_s *ctrl_shepherd) init() {
	ctrl_s.ctrl_musters = make(map[string]*ctrl_muster)
	for _, l_m := range(ctrl_s.local_musters) {
		intlog_m := intlog_muster{local_muster: *l_m}
		intlog_m.init()

		ctrl_m := ctrl_muster{intlog_muster: intlog_m}
		ctrl_m.init()

		ctrl_s.ctrl_musters[ctrl_m.id] = &ctrl_m
	}
}
