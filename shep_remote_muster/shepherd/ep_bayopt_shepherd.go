...
func (ep_s ep_shepherd) init() {
	for m_id, m := range(ep_s.musters) {
		...
		for c = 0; c < m.ncores; c++ {
			...
			ctrl_dvfs_c := control{id: ctrl_dvfs_id, n_ip: m.ip, knob: "dvfs", value: 0x1300, dirty: false}
...
func (ep_s *ep_shepherd) init_out_files(out_dir string) {
	for _, l_m := range(ep_s.local_musters) {
		...
		for _, sheep := range(l_m.pasture) {
			...
			out_fname := out_dir + "linux.mcd.dmesg.0_" + c_str + "_" + ctrl_itr + "_" + ctrl_dvfs + "_135_200000_" + l_m.id
...
func main() {
	...
	go ep_s.listen_heartbeats()
	go ep_s.process_logs()
	go ep_s.compute_control()
