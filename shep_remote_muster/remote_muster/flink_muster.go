...
func (energy_m *flink_energy_muster) init() {
	energy_m.flink_metrics = []string{"joules"}
	energy_m.buff_max_size = 1
	ctrl_itr_shared := control{n_ip: energy_m.ip, knob: "itr-delay", dirty: false}  
	rx_usecs_shared := ctrl_itr_shared.getter(0, read_rx_usecs)  

	for _, sheep := range(energy_m.pasture) {
		mem_buff := make([][]uint64, energy_m.buff_max_size)
		for _, log_c := range(sheep.logs) {
			log_c.metrics = energy_m.flink_metrics
			log_c.max_size = energy_m.buff_max_size
			log_c.mem_buff = &mem_buff
		}

		c_str := strconv.Itoa(int(sheep.core))
		ctrl_dvfs := control{n_ip: energy_m.ip, knob: "dvfs", dirty: false}
		ctrl_dvfs.id = "ctrl-dvfs-" + c_str + "-" + energy_m.ip
		ctrl_dvfs.value = ctrl_dvfs.getter(sheep.core, read_dvfs)
		ctrl_itr := ctrl_itr_shared
		ctrl_itr.id = "ctrl-itr-" + c_str + "-" + energy_m.ip
		ctrl_itr.value = rx_usecs_shared

		sheep.controls[ctrl_dvfs.id] = &ctrl_dvfs
		sheep.controls[ctrl_itr.id] = &ctrl_itr
	}
}
